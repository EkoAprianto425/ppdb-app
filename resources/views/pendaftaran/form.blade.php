            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <select name="provinsi" required class="w-full themed-input rounded-xl px-4 py-3 focus:border-primary focus:ring-1 focus:ring-primary/20 transition-all appearance-none">
                            <option value="">-- Pilih Provinsi --</option>
                            @foreach(['Jawa Barat', 'Jawa Tengah', 'Jawa Timur', 'DKI Jakarta', 'Banten', 'Bali'] as $prov)
                                <option value="{{ $prov }}" {{ old('provinsi', $registration->provinsi ?? '') == $prov ? 'selected' : '' }} class="text-slate-900">{{ $prov }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-xs font-medium themed-text-muted mb-2 uppercase tracking-wide">Kabupaten</label>
                        <select name="kabupaten" required class="w-full themed-input rounded-xl px-4 py-3 focus:border-primary focus:ring-1 focus:ring-primary/20 transition-all appearance-none">
                            <option value="">-- Pilih Kabupaten --</option>
                            @foreach(['Bandung', 'Garut', 'Tasikmalaya', 'Cianjur', 'Sumedang'] as $kab)
                                <option value="{{ $kab }}" {{ old('kabupaten', $registration->kabupaten ?? '') == $kab ? 'selected' : '' }} class="text-slate-900">{{ $kab }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-xs font-medium themed-text-muted mb-2 uppercase tracking-wide">Kecamatan</label>
                        <select name="kecamatan" required class="w-full themed-input rounded-xl px-4 py-3 focus:border-primary focus:ring-1 focus:ring-primary/20 transition-all appearance-none">
                            <option value="">-- Pilih Kecamatan --</option>
                            @foreach(['Ujung Berung', 'Cileunyi', 'Cibiru', 'Panyileukan'] as $kec)
                                <option value="{{ $kec }}" {{ old('kecamatan', $registration->kecamatan ?? '') == $kec ? 'selected' : '' }} class="text-slate-900">{{ $kec }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
